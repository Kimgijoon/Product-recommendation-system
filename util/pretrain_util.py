...
class PretrainHelper(object):
  ...
  def __init__(self, tokenizer_dir: str, config: collections.namedtuple):
    ...
    self.vocab = Vocab(os.path.join(tokenizer_dir, "tok.vocab"))

  def get_masked_lm_output(self, inputs, model):
    """mlm 결과 계산하는 함수
    Args:
      inputs: input_tensor
      model:  electra generator
    Return:
      MLMOutput:  several output
    """
    ...
    oh_labels = tf.one_hot(
        inputs.masked_lm_ids, depth=self.config.vocab_size,
        dtype=tf.float32)
  ...
  def get_discriminator_output(self, inputs, discriminator, labels):
    """Discriminator binary classifier."""
    with tf.variable_scope("discriminator_predictions"):
      hidden = tf.layers.dense(discriminator.get_sequence_output(),
                              units=self.config.hidden_size,
                              activation=modeling.get_activation(self.config.hidden_act),
                              kernel_initializer=modeling.create_initializer(self.config.initializer_range))
      ...
      DiscOutput = collections.namedtuple("DiscOutput",
  ...
  def get_fake_data(self, inputs, mlm_logits):
    """Sample from the generator to create corrupted input."""
    inputs = self.unmask(inputs)
    disallow = tf.one_hot(
        inputs.masked_lm_ids, depth=self.config.vocab_size,
        dtype=tf.float32) if self.config.disallow_correct else None
    sampled_tokens = tf.stop_gradient(self.sample_from_softmax(
        mlm_logits / self.config.temperature, disallow=disallow))
    sampled_tokids = tf.argmax(sampled_tokens, -1, output_type=tf.int32)
    updated_input_ids, masked = self.scatter_update(
        inputs.input_ids, sampled_tokids, inputs.masked_lm_positions)
    labels = masked * (1 - tf.cast(
        tf.equal(updated_input_ids, inputs.input_ids), tf.int32))
    updated_inputs = self.util.get_updated_inputs(
  ...
  def gather_positions(self, sequence: tf.Tensor, positions: tf.Tensor) -> tf.Tensor:
  ...
  def get_candidates_mask(self,
    ...
    ignore_ids = self.vocab.convert_tokens_to_ids(['[SEP]', '[CLS]', '[MASK]'])
  ...
  def mask(self,
    ...
    N = self.config.max_predictions_per_seq
    B, L = modeling.get_shape_list(inputs.input_ids)

    # Find indices where masking out a token is allowed
    candidates_mask = self.get_candidates_mask(inputs, disallow_from_mask)
    ...
    inputs_ids, _ = self.scatter_update(
        inputs.input_ids, tf.fill([B, N], self.vocab.convert_token_to_id('[MASK]')),
        replace_with_mask_positions)

    return self.util.get_updated_inputs(
        inputs,
  ...
  def unmask(self, inputs: collections.namedtuple):

    unmasked_input_ids, _ = self.scatter_update(
        inputs.input_ids, inputs.masked_lm_ids, inputs.masked_lm_positions)
    return self.util.get_updated_inputs(inputs, input_ids=unmasked_input_ids)

  def sample_from_softmax(self, logits, disallow=None):
